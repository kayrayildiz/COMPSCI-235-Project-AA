...
class Review:
    ...
    def __init__(self, book: 'Book', review_text: str, rating: int, user: User, timestamp: datetime = None):
        if isinstance(book, Book):
            self.__book = book
        else:
            self.__book = None
...
def make_review(book: Book, review_text: str, rating: int, user: User, timestamp: datetime):
    review = Review(book, review_text, rating, user, timestamp)
    # Creating the review is automatically relating the user and book (without calling add_review)
